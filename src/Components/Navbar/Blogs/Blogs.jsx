...
const Blogs = () => {
  ...
  // ✅ Item remove function to favourite 
  const handleRemoveFavorite = (id) => {
    setFavorites((prev) => prev.filter((item) => item.id !== id)); // create a new list without clicking 
    toast.error(" Removed from Favorites!"); // Showing toast 
  };

  return (
    <div className="overflow-x-auto my-10">
      <div className='text-black mx-[5.5%]'>
        <h1 className='text-3xl font-semibold  text-[#003EA4]'>Active <span className='text-[#FFD337] font-bold'>Auctions</span></h1>
        <p>Discover and bid on extraordinary items</p>
      </div>
      ...
      <div className="grid grid-cols-4 p-10 gap-6 mx-[3%]">
            ...
            <tbody>
              {blogs.map((blog) => (
                <Blog
                  key={blog.id}
                  blog={blog}
                  handleFavorite={handleFavorite} // Props from favourite function 
                />
              ))}
            </tbody>
          </table>
        ...
        {/* ❤️ Fovourite list for right side */}
        <div className="text-black bg-white h-auto shadow-md rounded-2xl text-center another-div flex flex-col gap-1">
          <div className="p-5">
            <p className="text-4xl font-bold border-b-2 border-gray-200 mb-8 py-5">
              ♡ <span className='text-4xl'>Favourite Items</span>
            ...
            {favorites.length === 0 ? (
              ...
              favorites.map((item) => (
                <div
                  ...
                  className="flex items-center justify-between px-4 py-3"
                >
                  <div className="flex items-center justify-between gap-3 bg-blue-50 p-5 shadow-2xl rounded-xl w-full border-1 border-black">
                    <img
                      src={item.image}
                      alt={item.title}
                      className="w-16 h-16 object-cover rounded-md bg-yellow-100 border-2 border-black"
                    />
                    <div className="text-left">
                      <h4 className="font-semibold text-base text-gray-800 truncate max-w-[200px]">
                        {item.title}
                      ...
                      <div className="text-sm text-gray-600 mt-1">
                        Bids: {item.currentBidPrice} {item.totalBids}
                      </div>
                    </div>

                    {/* ❌ এই img ক্লিক করলে ফেভারিট থেকে remove হবে */}
                    <img
